...
//! `u16` access.
//!
//! Generated by `scripts/gen-omaha-noflush-lookup`. Stored as a
//! little-endian `u16` binary blob and exposed through `include_bytes!`
//! to keep compile times sane (a `const [u16; ~11M]` literal would
//! make `rustc` crawl).
...
static ALIGNED: &Aligned<[u8; BLOB_SIZE]> = &Aligned {
    _align: [],
    bytes: *include_bytes!("noflush_lookup.bin"),
};
