...
pub fn evaluate_plo4_cards(c1: i32, c2: i32, c3: i32, c4: i32, c5: i32,
    ...
    for i in 0..4 {
        ...
        if scb >= 3 && sch >= 2 {
            ...
            if scb == 3 && sch == 2 {
                ...
                let board_padded = sbb | unsafe {
                    *PADDING.get_unchecked((5 - scb) as usize)
                };
                let hole_padded = sbh | unsafe {
                    *PADDING.get_unchecked((4 - sch) as usize)
                };
                ...
                value_flush = unsafe {
                    *FLUSH_PLO4.get_unchecked((board_hash * 1365 + hole_hash) as usize)
                } as i32;
            }
    ...
    value_noflush = unsafe {
        *NOFLUSH_PLO4.get_unchecked((board_hash * 1820 + hole_hash) as usize)
    } as i32;
...
mod tests {
    ...
    fn royal_straight_flush() {
        let r = evaluate_plo4_cards(
            QUEEN_S, JACK_S, TEN_S, TWO_C, THREE_C,
            ACE_S, KING_S, TWO_D, TWO_H,
        );
        assert_eq!(r, 1);
    }
    ...
    fn pair_of_aces_ranges() {
        let r = evaluate_plo4_cards(
            KING_S, FIVE_C, FOUR_C, THREE_C, TWO_C,
            ACE_C, ACE_D, ACE_H, ACE_S,
        );
        assert!(r >= 3326 && r <= 6185);
    }

    #[test]
    fn flush_5card_special_case() {
        let r = evaluate_plo4_cards(
            card(3, 2), card(4, 2), card(5, 2), TWO_C, FOUR_C,
