...
//!    The answer depends only on the rank multisets of hole and
//!    board, so we collapse the 60-combo enumeration into **one**
//!    `NOFLUSH_LOOKUP` access. The lookup is keyed by a flat
//!    `hole_idx * NUM_BOARD + board_idx`, where each side's index
//!    comes from the multiset combinatorial number system over its
//!    sorted rank list. The 22 MB table is generated by
//!    `scripts/gen-omaha-noflush-lookup` and lives in
//!    `phe-omaha-assets`.
//!
//! 2. **Flush-dominates path (9-card direct).** A flush is reachable
//!    AND the board has 5 distinct ranks (i.e., no pair on board).
//!    On a no-pair board, FullHouse and Quads are mathematically
//!    unreachable for *any* combo, so the best 5-card hand is the
//!    best flush (or SF). We compute the answer with **one**
//!    `LOOKUP_FLUSH` access: a 10-window straight-flush scan over
//!    per-suit rank bitmasks (highest SF window that has ≥2 hole and
//!    ≥3 board ranks wins), then top-2 hole / top-3 board bit-OR for
//!    the plain-flush fallback. No (hole_pair × board_triple) inner
//!    enumeration.
//!
...
/// Pre-computed binomial table `BINOM[a][b] = C(a, b)` covering the
/// values needed by `hole_index` / `board_index` (`a <= 16, b <= 5`).
/// Hand-computed `const fn`-free table to avoid any `OnceLock`-style
/// initialisation overhead in the hot path.
...
/// In-place 4-element ascending sort via a 5-comparator network
/// (Bose-Nelson optimal). Branchless when the comparisons compile
/// to `cmov`, which they do on x86_64 in release mode.
#[inline]
...
/// In-place 5-element ascending sort via a 9-comparator network.
#[inline]
...
///
/// Construction: with sorted ranks `r0 ≤ r1 ≤ r2 ≤ r3` in `[0, 13)`,
/// shift by position to get a strict-increasing sequence
/// `y_i = r_i + i` and sum the standard combinatorial number system
/// terms `C(y_0, 1) + C(y_1, 2) + C(y_2, 3) + C(y_3, 4)`. The map
/// is bijective onto `[0, C(16, 4))`.
#[inline]
...
/// Same as `hole_index` but for the board's 5 rank values. Returns
/// dense index in `[0, 6188)`.
#[inline]
...
///
/// Single 22 MB table access. Hole and board are encoded into dense
/// multiset indices via the combinatorial number system; the flat
/// table holds the 60-combo max non-flush rank pre-computed for every
/// reachable `(hole multiset, board multiset)` pair.
#[inline]
fn evaluate_no_flush_path(hole: &[usize; 4], board: &[usize; 5]) -> u16 {
    let h_idx = hole_index(hole);
    let b_idx = board_index(board);
    let key = h_idx * NUM_BOARD + b_idx;
    unsafe { *noflush_lookup().get_unchecked(key) }
}

/// 10 straight-flush 5-rank windows in descending rank order. Index 0
/// is T-J-Q-K-A (royal SF, highest), index 8 is 2-3-4-5-6 (lowest
/// "standard" SF), index 9 is the wheel A-2-3-4-5 (lowest SF overall,
/// still beats any plain flush per the LOOKUP_FLUSH table).
...
/// Returns `mask` with all but its top-2 set bits cleared. Caller
/// must ensure `mask.count_ones() >= 2`.
#[inline]
...
/// Returns `mask` with all but its top-3 set bits cleared. Caller
/// must ensure `mask.count_ones() >= 3`.
#[inline]
...
/// Flush-dominates path, 9-card direct version: pick the answer with
/// **one** `LOOKUP_FLUSH` access total.
///
...
///
/// Algorithm:
///   1. Build per-suit rank bitmasks `hole_mask` / `board_mask` for
///      cards in `suit`. These are the 13-bit "rank patterns"
///      restricted to the flush suit.
///   2. Scan 10 straight-flush 5-rank windows from highest (royal) to
///      lowest (wheel). The window `W` is reachable as a 5-card SF
///      under Omaha's 2+3 rule iff
///        `popcount(hole_mask & W) >= 2 && popcount(board_mask & W) >= 3`.
///      The first matching window is the strongest reachable SF —
///      `LOOKUP_FLUSH[W]` is the answer. Why this is correct: hole
///      and board are disjoint sets of cards in the same suit, so
///      `(hole_mask & W) ∩ (board_mask & W) = ∅`. With both
///      counts ≥ thresholds and a disjoint sum bounded by `|W| = 5`,
///      the sums must equal exactly 2 and 3, fully covering `W` so
///      the SF combo really is buildable.
///   3. If no SF window matched, the best plain flush is
///      `top-2 hole-suit bits | top-3 board-suit bits`. Greedy
///      argument: each contributing rank slot is independently
///      maximized, the union has 5 distinct ranks (hole/board
///      disjoint in suit), and LOOKUP_FLUSH ranks plain flushes by
///      the sorted 5-rank pattern. One LOOKUP_FLUSH access at the
///      computed key.
///
/// Replaces an earlier enumeration of `C(hole_S, 2) × C(board_S, 3)`
/// combos that did up to 30 LOOKUP_FLUSH accesses; this version
/// always does exactly 1.
fn evaluate_flush_dominate(
    ...
    // SF window scan, highest-rank first.
    ...
    // No SF: best plain flush is top-2 hole-suit | top-3 board-suit.
